...
Both hosts read the same [agent workflow](prompts/agent_workflow.md): repository questions stay outside research startup, existing approvals remain scoped to the reviewed work, and independent scientific review retains its evidence gates. Host adapters use available tools and skills; missing optional skills have explicit manual procedures. Model choice stays in your host settings. See the [validation procedure](docs/cross-host-validation.md) and [recorded results](docs/cross-host-validation-results.md) for scenario coverage, tested host details, and remaining limits. Synthetic decision checks do not establish equivalent research quality or operational behavior.
