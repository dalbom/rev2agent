...
Shared execution rules live in `prompts/agent_workflow.md`; shared scientific protocols stay in `prompts/`. This section describes Claude Code mechanics only.

- Use the tools and agent mechanisms exposed by the current Claude Code host. Use the Agent tool or custom agent types only when available; translate legacy `Task` wording to the supported mechanism.
- For bounded independent work, use isolated Claude agents with the phase's role and output contract. Keep launches and research-state writes with the main agent. If a required independent reviewer is unavailable, follow the pending-gate rule in the shared workflow.
- `research-deep-dive`, `code-quality-review`, and `writing-humanizer` may map to installed `/deep-research`, `/simplify`, and `/humanizer` equivalents. Check availability and instructions first; otherwise perform the shared wrapper procedure directly. These commands are not required repository dependencies.
- Use supported background work and user input handling to continue independent work while a question or tool is pending. If the host only supports sequential turns, check for steering at the next available boundary and preserve the original objective.
